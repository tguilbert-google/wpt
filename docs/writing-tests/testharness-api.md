...
promise_test(function(t) {
  return promise_rejects_js(t, new TypeError(), bar());
}, "Another example");
```

```js
promise_rejects_dom(test_object, expected, promise, description)
```
assert the promise rejects with a DOMException of the expected type.

`type` - The expected exception name or code.  See the table of names
and codes at https://heycam.github.io/webidl/#dfn-error-names-table If
a number is passed it should be one of the numeric code values in that
